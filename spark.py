...
# spark = SparkSession.builder \
#     .appName("Optimized Property Price Prediction") \
#     .config("spark.executor.memory", "2g") \
#     .config("spark.driver.memory", "4g") \
#     .getOrCreate()

# print("Starting optimized property price analysis...")
...
# string_cols = [field.name for field in df.schema.fields 
#               if field.dataType.typeName() == "string"]
# print("Found " + str(len(string_cols)) + " categorical columns")
...
#                if col_name != target_col and col_name != log_target_col and col_name not in string_cols]
# print("Using " + str(len(feature_cols)) + " numeric features")
...
# # Créer les indexeurs pour les colonnes catégorielles
# indexers = [StringIndexer(inputCol=col_name, outputCol=col_name + "_indexed", handleInvalid="keep") 
#            for col_name in string_cols]
# print("Created indexers for " + str(len(indexers)) + " categorical columns")

# # Créer l'assembleur de vecteurs
# assembler = VectorAssembler(
#     inputCols=feature_cols + [c + "_indexed" for c in string_cols], 
#     outputCol="features", 
#     handleInvalid="skip"
...
# # Créer la grille de paramètres pour GBT
...
# print("Training GBT model with cross-validation (this may take a while)...")
...
# print("Cross-validation complete. Best average R² score: {:.4f}".format(max(gbt_cv_model.avgMetrics)))

# # Faire des prédictions
# print("Making predictions on test set...")
# predictions = best_gbt_model.transform(test_df)

# # Convertir les prédictions logarithmiques en échelle originale
# predictions = predictions.withColumn("exp_prediction", F.expm1(predictions["prediction"]))

# # Calculer les métriques sur l'échelle logarithmique
# print("Evaluating model...")
# r2_log = evaluator.evaluate(predictions)
...
#     metricName="rmse"
# )
# rmse_log = rmse_evaluator.evaluate(predictions)
...
#     metricName="mae"
# )
# mae_log = mae_evaluator.evaluate(predictions)

# # Calculer les métriques sur l'échelle originale
# r2_evaluator_orig = RegressionEvaluator(
#     labelCol=target_col, 
#     predictionCol="exp_prediction", 
#     metricName="r2"
# )
# r2_original = r2_evaluator_orig.evaluate(predictions)

# rmse_evaluator_orig = RegressionEvaluator(
#     labelCol=target_col, 
#     predictionCol="exp_prediction", 
#     metricName="rmse"
# )
# rmse_original = rmse_evaluator_orig.evaluate(predictions)
...
# try:
#     print("\n===== FEATURE IMPORTANCE =====")
#     gbt_model = best_gbt_model.stages[-1]
#     importances = gbt_model.featureImportances
...
# # Mesure de l'écart entre les performances CV et test
# avg_cv_score = max(gbt_cv_model.avgMetrics)
# test_score = r2_log
# score_difference = abs(avg_cv_score - test_score)

# print("\nModel stability analysis:")
# print("Best cross-validation R2: {:.4f}".format(avg_cv_score))
# print("Test set R2: {:.4f}".format(test_score))
# print("Absolute difference: {:.4f}".format(score_difference))

# # Résumé des résultats
# print("\n======= SUMMARY REPORT =======")
# print("Dataset: {} rows, {} columns".format(df.count(), len(df.columns)))
# print("Performance (log scale): R2={:.4f}, RMSE={:.4f}, MAE={:.4f}".format(r2_log, rmse_log, mae_log))
# print("Performance (original scale): R2={:.4f}, RMSE={:.4f}".format(r2_original, rmse_original))
# print("Adjusted R2: {:.4f}".format(adjusted_r2))
# print("Top 3 important features: {}".format(", ".join([f[0] for f in feature_importance[:3]])))
